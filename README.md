...
6. controller层采用map + warpper方式的返回结果，返回给前端更为灵活的数据，具体参见com.yaluzb.guns.modular.system.warpper包中具体类。
...
9. 控制器层统一的异常拦截机制,利用@ControllerAdvice统一对异常拦截,具体见com.yaluzb.guns.core.aop.GlobalExceptionHandler类。
...
```
@Configuration
@MapperScan(basePackages = {"com.yaluzb.guns.modular.*.dao", "com.yaluzb.guns.common.persistence.dao"})
...
## 对js常用代码的封装
在webapp/static/js/common目录中，有对常用js代码的封装，例如yaluzb.js,其中yaluzb.info(),yaluzb.success(),yaluzb.error()三个方法，分别封装了普通提示，成功提示，错误提示的代码，简化了layer提示层插件的使用。
