...
MenuInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/menu/add", function (data) {
        yaluzb.success("添加成功!");
...
MenuInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/menu/edit", function (data) {
        yaluzb.success("修改成功!");
        window.parent.Menu.table.refresh();
        MenuInfoDlg.close();
    }, function (data) {
...
MenuInfoDlg.showMenuSelectTree = function () {
    yaluzb.showInputTree("pcodeName", "pcodeTreeDiv", 15, 34);
};

$(function () {
    yaluzb.initValidator("menuInfoForm", MenuInfoDlg.validateFields);
