...
NoticeInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/notice/add", function (data) {
        yaluzb.success("添加成功!");
...
NoticeInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/notice/update", function (data) {
        yaluzb.success("修改成功!");
        window.parent.Notice.table.refresh();
        NoticeInfoDlg.close();
    }, function (data) {
...
$(function () {
    yaluzb.initValidator("noticeInfoForm", NoticeInfoDlg.validateFields);
