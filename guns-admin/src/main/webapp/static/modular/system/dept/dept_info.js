...
DeptInfoDlg.addSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/dept/add", function(data){
        yaluzb.success("添加成功!");
...
DeptInfoDlg.editSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/dept/update", function(data){
        yaluzb.success("修改成功!");
        window.parent.Dept.table.refresh();
        DeptInfoDlg.close();
    },function(data){
...
$(function() {
    yaluzb.initValidator("deptInfoForm", DeptInfoDlg.validateFields);
