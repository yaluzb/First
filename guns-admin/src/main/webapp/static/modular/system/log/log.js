...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(yaluzb.ctxPath + "/log/detail/" + this.seItem.id, function (data) {
            yaluzb.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            yaluzb.error("获取详情失败!");
        });
        ajax.start();
...
OptLog.delLog = function () {
    yaluzb.confirm("是否清空所有日志?",function(){
        var ajax = yaluzb.baseAjax("/log/delLog","清空日志");
        ajax.start();
