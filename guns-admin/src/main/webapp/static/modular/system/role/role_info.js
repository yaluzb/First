...
RolInfoDlg.showDeptSelectTree = function () {
    yaluzb.showInputTree("deptName", "deptContent");
};
...
RolInfoDlg.showPNameSelectTree = function () {
    yaluzb.showInputTree("pName", "pNameContent");
};
...
RolInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/role/add", function (data) {
        yaluzb.success("添加成功!");
        window.parent.Role.table.refresh();
        RolInfoDlg.close();
    }, function (data) {
        yaluzb.error("添加失败!" + data.responseJSON.message + "!");
    });
...
RolInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(yaluzb.ctxPath + "/role/edit", function (data) {
        yaluzb.success("修改成功!");
        window.parent.Role.table.refresh();
        RolInfoDlg.close();
    }, function (data) {
        yaluzb.error("修改失败!" + data.responseJSON.message + "!");
    });
    ajax.set(this.roleInfoData);
    ajax.start();
};
...
$(function () {
    yaluzb.initValidator("roleInfoForm", RolInfoDlg.validateFields);
