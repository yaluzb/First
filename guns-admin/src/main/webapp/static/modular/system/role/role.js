...
Role.openAddRole = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: yaluzb.ctxPath + '/role/role_add'
    });
...
Role.openChangeRole = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: yaluzb.ctxPath + '/role/role_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Role.delRole = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(yaluzb.ctxPath + "/role/remove", function () {
                yaluzb.success("删除成功!");
                Role.table.refresh();
            }, function (data) {
        ...
        yaluzb.confirm("是否删除角色 " + Role.seItem.name + "?",operation);
    }
};
...
Role.assign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: yaluzb.ctxPath + '/role/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
