...
Process.pass = function (id) {
    var ajax = new $ax(yaluzb.ctxPath + "/process/pass", function (data) {
        yaluzb.success("审核成功!");
        Process.table.refresh();
    }, function (data) {
        yaluzb.error("审核失败!" + data.responseJSON.message + "!");
    });
    ajax.set("taskId", id);
    ajax.start();
};
...
Process.unPass = function (id) {
    var ajax = new $ax(yaluzb.ctxPath + "/process/unPass", function (data) {
        yaluzb.success("审核成功!");
        Process.table.refresh();
    }, function (data) {
        yaluzb.error("审核失败!" + data.responseJSON.message + "!");
